...
import org.infinispan.marshaller.kryo.KryoTranscoder;
...
final class CompatibilityCacheFactory<K, V> {
   ...
   private MemcachedClient createMemcachedClient(long timeout, int port) throws IOException {
      ...
      if (marshaller instanceof KryoMarshaller) {
         KryoMarshaller protoMarshaller = (KryoMarshaller) marshaller;
         cf = new ConnectionFactoryBuilder(cf).setTranscoder(new KryoTranscoder(protoMarshaller)).build();
      }
